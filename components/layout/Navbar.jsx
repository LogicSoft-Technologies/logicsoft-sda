...
function SearchOverlay({ query, onClose }) {
  ...
  useEffect(() => {
    const handle = (e) => {
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setActive((i) => Math.min(i + 1, flatHrefs.length - 1));
      }
      if (e.key === "ArrowUp") {
        e.preventDefault();
              ...
              <div key={category}>
                ...
                {items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="w-[3px] self-stretch shrink-0 mt-0.5 rounded-full"
                        style={{
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-[13.5px] font-semibold leading-snug ${isActive ? "text-[#1f6fb2]" : "text-[#1f3a5f]"}`}
                        >
                          {matchAt >= 0 ? (
                            <>
                      ...
                      <ArrowRight
                        className={`w-3.5 h-3.5 mt-1 shrink-0 transition-opacity ${isActive ? "opacity-100 text-[#1f6fb2]" : "opacity-0"}`}
                      />
                    </button>
                  );
                })}
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e8eef6] bg-[#f8fafc] shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-[12px] font-bold text-[#1f3a5f] leading-none">
                    LogicSoft
                  </p>
                  <p className="text-[8.5px] font-semibold text-[#1f6fb2] uppercase tracking-[0.15em] leading-none mt-0.5">
                    Technologies
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <nav aria-label="Mobile navigation">
                  ...
                  return (
                    <div key={key} className="border-b border-[#f1f5f9]">
                      ...
                      >
                        <span>{label}</span>
                        <ChevronDown
                          className={`w-4 h-4 transition-transform duration-200 ${isOpen ? "rotate-180 text-[#1f6fb2]" : "text-gray-400"}`}
                        />
                      </button>
                      ...
                      <AnimatePresence initial={false}>
                        {isOpen && (
                          ...
                          >
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                <p className="text-[11.5px] font-bold text-[#1f6fb2] mb-0.5">
                                  {dd.featured.label}
                                </p>
                            ...
                            {dd.columns.map((col) => (
                              <div key={col.title} className="px-4 mb-4">
                                <p className="text-[9.5px] font-bold text-gray-400 uppercase tracking-[0.15em] mb-1.5 px-1">
                                  {col.title}
                                </p>
                ...
                <div className="px-5 pt-4 pb-2">
                  <p className="text-[9.5px] font-bold text-gray-400 uppercase tracking-[0.15em] mb-3">
                    Quick links
                  </p>
                ...
                <div className="mx-5 my-4 p-3 bg-[#f8fafc] border border-[#e8eef6]">
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-[0.14em] mb-2">
                    Certifications
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {["ISO 27001", "GDPR", "NDPR", "SOC 2"].map((c) => (
                      <span
                        key={c}
                        className="text-[9.5px] font-bold text-[#1f3a5f] border border-[#e8eef6] bg-white px-2 py-0.5"
                      >
                        {c}
                      </span>
                    ))}
            ...
            <div className="shrink-0 border-t border-[#e8eef6] bg-white p-4 space-y-2.5">
              <Link
                ...
                style={{
                  ...
                  boxShadow: "0 4px 16px rgba(196,85,0,0.3)",
                }}
              >
      ...
      {query.trim() && results.length > 0 && (
        <div>
          {results.slice(0, 8).map((item) => {
            ...
            return (
              ...
              >
                ...
                />
                <div className="flex-1 min-w-0">
                  <p className="text-[13.5px] font-semibold text-[#1f3a5f] truncate">
                    {item.title}
                  </p>
          ...
          {results.length > 8 && (
            <button
              onClick={goSearch}
              className="w-full py-3 text-[12.5px] font-bold text-[#1f6fb2] hover:text-[#1f3a5f] hover:bg-[#f8fafd] transition-colors text-center"
            >
      ...
      {query.trim() && results.length === 0 && (
        <div className="py-8 text-center">
          ...
          <Link
            href="/services"
            onClick={onClose}
            className="text-[12.5px] font-bold text-[#1f6fb2]"
          >
            Browse all services →
          </Link>
        </div>
...
const Navbar = () => {
  ...
  const isBlogArticle = pathname.startsWith("/blog/") && !isBlogHero;

  const isDropdownOpen = clicked !== "";

  const showSolid =
    (!isHome && !isBlogHero) || scrolled || isDropdownOpen || mobileOpen;
  ...
  // pages are meant to feel like a full-bleed cinematic piece, not a
  // subpage with a header sitting on top of it.
  ...
  // All hooks above still run on every render regardless of route — this
  // early return only skips the *output*, so hook order never changes.
  ...
  return (
    <>
      ...
      >
        <div
          className={`hidden md:block transition-colors duration-300 ${showSolid ? "bg-[#1f3a5f]" : "bg-transparent"}`}
        >
          <div
            className={`max-w-[82rem] mx-auto px-4 py-[5px] flex items-center gap-6 text-[11.5px] transition-colors duration-300 ${showSolid ? "text-white/70" : isBlogHero ? "text-white/70" : "text-gray-800"}`}
          >
            <a
              href="mailto:contact@logicsofttechnologies.com"
            ...
            <div className="ml-auto flex items-center gap-5">
              <span className={showSolid ? "text-white/30" : "text-gray-300"}>
                |
              </span>
              <Link
                href="/portfolio"
        ...
        <div
          className={`relative transition-all duration-300 border-b ${isDropdownOpen ? "border-gray-200" : "border-transparent"} ${showSolid ? "bg-white" : "bg-transparent"}`}
        >
          <div className="max-w-[82rem] mx-auto px-4 py-3 flex items-center">
            <Link
              href="/"
              aria-label="LogicSoft Technologies — Home"
              className="flex items-center shrink-0 mr-6"
            >
              <Image
                src={
                  !showSolid && isBlogHero
                    ? "/images/logicsoft-logo-white.png"
                    : "/images/logicsoft-logo.png"
                }
            ...
            <div className="hidden md:flex items-center gap-1 ml-auto">
              {NAV_LINKS.map((item) => {
                ...
                const hasDropdown =
                  item.key === "about" || item.key === "services";
                ...
                return hasDropdown ? (
                  ...
                  >
                    {item.label}
                    <ChevronDown
                      className={`h-3.5 w-3.5 transition-transform duration-250 ${isActive ? "rotate-180 text-[#1f6fb2]" : showSolid ? "text-gray-400" : transparentMuted}`}
                    />
                    {isActive && (
                      <motion.span
                  ...
                  <Link
                    ...
                    className={`px-3 py-2 text-[14px] font-medium transition-colors duration-150 ${
                      showSolid
                        ? "text-gray-700 hover:text-[#1f6fb2]"
                        : `${transparentText} ${transparentHover}`
                    }`}
                  >
                    {item.label}
                  </Link>
              ...
              <div className="relative inline-flex items-center justify-center">
                {[0, 0.2].map((delay, i) => (
                  <motion.span
                    ...
                    style={{
                      border: `${i === 0 ? 2 : 1}px solid rgba(196,85,0,${i === 0 ? 0.65 : 0.4})`,
                    }}
                    initial={{ scale: 0.96, opacity: 0 }}
                    animate={{
                      scale: [0.96, 1.32, 1.32],
                      opacity: [0, 0.6, 0],
                    }}
                    transition={{
                      duration: i === 0 ? 1.5 : 2,
              ...
              <div ref={searchWrapRef} className="relative ml-2">
                ...
                <AnimatePresence>
                  {searchOpen && (
                    <SearchOverlay query={query} onClose={closeSearch} />
                  )}
                </AnimatePresence>
              </div>
            </div>
            ...
            >
              <div className="max-w-[82rem] mx-auto px-4 py-8 grid grid-cols-5 gap-8">
                ...
                {DROPDOWNS[clicked].columns.map((col, ci) => (
                  <div
                    key={col.title}
                    className={ci > 0 ? "border-l border-gray-100 pl-6" : ""}
                  >
                  ...
                  <div className="ml-auto flex items-center gap-4 text-[12px] text-gray-400">
                    ...
                    {[
                      ...
                      <Link
                        key={s}
                        href="#"
                        className="hover:text-[#1f6fb2] transition-colors duration-150 underline underline-offset-2"
                      >
                        {s}
                      </Link>
                    ))}
